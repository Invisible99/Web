...
class forum_model extends MY_Model{
    ...
    function findSubforumsNoThread(){
        ...
        $query = $this->db->query("SELECT cat.categorieID, cat.titel, cat.omschrijving, cat.magZienTot FROM categories cat LEFT JOIN threads thr ON cat.categorieID = thr.categorieID LEFT JOIN posts ON thr.topicID = posts.topicID LEFT JOIN users ON posts.gebruikerID = users.gebruikerID
WHERE
